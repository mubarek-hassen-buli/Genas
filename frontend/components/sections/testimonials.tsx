...
export function Testimonials() {
  ...
  return (
    <section className="py-24 bg-[#f8fafc]/50 flex flex-col items-center justify-center text-center px-4 overflow-hidden">
      {/* Header */}
      ...
      <div className="w-full max-w-7xl mx-auto flex flex-col lg:flex-row items-stretch gap-6 px-4 mb-16">
        {testimonials.map((item, index) => {
          ...
          <div 
            ...
            className={`flex flex-col text-left p-6 sm:p-8 rounded-[32px] bg-gradient-to-b from-white to-[#e2ecff] shadow-sm relative min-h-[300px] cursor-pointer transition-all duration-500 ease-in-out ${
              isActive ? "lg:flex-[1.5] shadow-lg scale-[1.02] border border-blue-100" : "lg:flex-1 hover:bg-opacity-80"
            }`}
          >
             <h4 className="text-[13px] text-slate-400 font-medium mb-6 transition-colors">{item.role}</h4>
             <p className="text-[15px] sm:text-base text-slate-800 font-medium leading-relaxed whitespace-pre-wrap flex-1">
               {item.text}
             </p>

             <div className={`flex items-center gap-1 mt-6 transition-all duration-500 origin-left ${isActive ? 'opacity-100 scale-100' : 'opacity-0 scale-95 h-0 overflow-hidden'}`}>
               {[...Array(5)].map((_, i) => (
